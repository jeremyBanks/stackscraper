...
StackScraper = (function() {
  ...
  StackScraper.prototype.getPostComments = function(postid) {
    ...
    return this.ajax("/posts/" + postid + "/comments").pipe(function(commentsSource) {
      var commentPage$;
      return commentPage$ = $(makeDocument("<body><table>" + commentsSource + "</table></body>"));
    });
  };
...
$('#question .post-menu').append('<span class="lsep">|</span>').append($('<a href="#" title="download a JSON copy of this post">download</a>').click(function() {
  var questionId;
  ...
  stackScraper.getQuestion(questionId).then(function(question) {
    ...
    return window.location = URL.createObjectURL(bb.getBlob());
  });
  return false;
}));
